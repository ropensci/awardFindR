...
```
fedreporter <- fedreporter_get(query="keyword", from=2015, to=2020, agency="usda,dod")
```
...
```
nih <- nih_get(query="keyword", from="2018-01-01", to="2020-01-01")
```

Like NSF and the Federal Reporter, because of the nature of the API, the direct query function only handles one keyword at a time. Multiple keywords are handled at a higher level.
...
```
ophil_get("qualitative", 2019, 2020)
```
