...
NSF operates through the `api_scrape()` routine. To fetch two years of data, where `keywords` is a character vector, run the following example:

```
api_scrape(queries=keywords, from="2018-01-01", to="2020-01-01", sources="nsf")
```
...
The Federal Reporter operates through the `api_scrape()` routine. To fetch two years of data for NIH and IES, where `keywords` is a character vector, run the following example:

```
api_scrape(queries=keywords, from="2018-01-01", to="2020-01-01", sources=c("nih", "ies"))
```
...
While the `sloan_df()` function can be run independently to construct the a full data.frame from their website, like so
...
```
static_scrape(queries=keywords, from="2018-01-01", to="2020-01-01", sources="sloan")
